...
try {
  ...
  // short timeout so Stop/PostToolUse cannot stall the ZCode turn.
...
function logHookInvoke(payload, error = null) {
  if (!error && process.env.ATOLL_ZCODE_HOOK_DEBUG !== "1") {
    return;
  }
  ...
  try {
    ...
    fs.appendFileSync(
      path.join(base, "zcode-hook-invoke.log"),
...
function fallbackResponse(hookEventName, error) {
  // ZCode falls back to its own permission prompt when a hook prints nothing,
  // so observer events and PermissionRequest degrade to "{}" instead of
  // blocking the session on an unavailable Atoll bridge.
  if (
