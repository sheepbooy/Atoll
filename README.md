...
- **审批规则** — Settings → Approval Rules 配置持久化的自动放行/拦截规则（按 Agent、工具、命令模式 `*`/`?` 通配符、项目路径匹配）；审批卡「Always ▾」可当场把当前命令存为规则；默认开启**风险命令保护**，`rm -rf`、`sudo`、强制推送等危险命令永远不会被规则自动放行，规则命中的历史记录单独标记、可筛选
- **通知中心提醒** — Settings → Notifications 可切换提醒模式：默认**强制打断**（浮岛自动展开并置前）；**仅通知**模式不打断当前工作，发送系统通知，点击通知或浮岛即展开审批
- **全局快捷键** — Settings → Shortcuts 为「召回浮岛 / 批准 / 拒绝 / 始终允许」配置全局快捷键，听到通知声后无需切换窗口即可一键处理
